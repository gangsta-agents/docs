...
In Autonomous Mode (started via `/gangsta:heist`), Laundering runs with the Don Proxy approving the final declaration. Ledger entries are written with `signed-by: don-proxy` and `status: pending-don-confirmation`. The `/gangsta:go` command exits after Laundering completes. [Full Autonomous Mode docs →](/advanced/autonomous-mode)
